...
def warmup_pixel_module_an_iter(config, base, loaders):
	...
	# fake_rgb_masks = torch.cat((fake_rgb_masks,fake_rgb_masks,fake_rgb_masks),dim=1)

	fake_ir_masks = base.G_rgb2ir(real_rgb_images)
	# fake_ir_masks = torch.cat((fake_ir_masks,fake_ir_masks,fake_ir_masks),dim=1)

	# fake images (with threshold)
	...
	# fake_rgb_images = torch.mul(fake_rgb_masks, real_ir_images)
	# fake_rgb_images = map_2_tanh(fake_rgb_images)
	#
